...
{ lib, mkRocqDerivation, which, coq
  ...
  , mathcomp-boot, stdlib
  , version ? null }:

with lib; mkRocqDerivation {
  pname = "RocqHigmanOrder";
  repo = "RocqHigmanOrder";
  owner = "reiniscirpons";
  domain = "github.com";
...
## - or its version is overridden (by a branch, PR, url or path) in `.nix/config.nix`.
  ...
  propagatedBuildInputs = [ stdlib mathcomp-boot ];

  ## Does the package contain OCaml code?
  mlPlugin = false;

  meta = {
    description = "A formalization of the theory of well-quasi-orders in Rocq";
    maintainers = with maintainers; [ reiniscirpons ];
